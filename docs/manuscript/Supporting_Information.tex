...
\noindent\textbf{Remediation note (2026-09).}
Main-text Table~IV LOLO affinity metrics are reconciled to
RMSE\,$\approx$\,1.05\,kcal/mol and Spearman $\rho \approx 0.16$
(vs.\ trivial baselines 0.992 / 0.918); prior $\rho \approx 0.93$ does
not reproduce.
NHS and NHS-ester structures have been re-docked; PGG re-docking and
schema-2 LOLO re-train remain in progress---see
\texttt{docs/REMEDIATION\_STATUS.md} in the code repository.
SI appendices that quote older CSI Spearman values should be read as
historical until recomputed.
...
\textbf{PGG} & $^{***}$ & $^{***}$ & $^{***}$ & $^{***}$ & $^{***}$ & $^{***}$ & $^{***}$ & $^{***}$ & --- \\
\bottomrule
\multicolumn{10}{l}{\small Upper triangle: mean $\Delta G$ difference (kcal/mol); lower triangle: Tukey-corrected $p$-value ($^{***} p < 0.001$).}
\end{tabular}
\end{table}
...
\midrule
\multicolumn{8}{l}{\small Between-group ANOVA: $F_{2,6153} = 1305.45$, $p < 10^{-300}$, $\eta^2 = 0.298$.} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Model~D} trains on a smaller subset (5,267 samples) due
    to the MMP-1 data requirement. Previously reported CSI Spearman
    $\rho = 0.95$ is \emph{not} validated under the same LOLO protocol as
    the main-text affinity metrics (Table~IV of the main manuscript;
    LOLO $\rho \approx 0.16$) and must be recomputed after B6 ligand
    re-docking and MMP-1 box remediation.
\end{itemize}
...
\textbf{Ligand} & $N$ & $\bar{I}$ & $I_{\max}$
  & $\sigma_I$ & $\delta$ \\
\midrule
EDC                  & 11 & 0.090 & 0.358 & 0.087 & $-0.160$ \\
EDC $O$-acylisourea  & 16 & 0.043 & 0.081 & 0.013 & $-0.072$ \\
NHS                  &  7 & 0.153 & 0.233 & 0.055 & $-0.134$ \\
NHS ester interm.    & 11 & 0.050 & 0.096 & 0.024 & $-0.063$ \\
\midrule
Gallic acid          & 12 & 0.439 & 0.666 & 0.172 & $+0.692$ \\
Protocatechuic acid  & 11 & 0.429 & 0.744 & 0.181 & $+0.580$ \\
Pyrogallol           &  9 & 0.295 & 0.538 & 0.163 & $+0.430$ \\
Ellagic acid         & 22 & 0.349 & 0.477 & 0.093 & $+1.045$ \\
PGG                  & 81 & 0.097 & 0.216 & 0.041 & $+1.066$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=1.5em]
  \item GA analogues show 3--10$\times$ higher mean atom importance
    than crosslinkers, confirming that Model~D recognises the
    galloyl pharmacophore as the primary binding determinant.
  \item Gallic acid has the highest per-atom importance ($\bar{I} = 0.439$)
    despite having only 12 heavy atoms, suggesting concentrated
    binding contributions from each atom.
  \item PGG's lower per-atom importance ($\bar{I} = 0.097$) reflects
    its large size (81 atoms) --- the total molecular attribution
    ($81 \times 0.097 = 7.86$) is actually the highest of all ligands.
  \item Positive convergence deltas for GA analogues and negative
    for crosslinkers may indicate opposite gradient directions
