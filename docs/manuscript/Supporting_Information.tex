...
Main-text Table~IV LOLO affinity metrics (post--B6 schema-2 re-train)
are RMSE\,$= 1.338$\,kcal/mol, MAE\,$= 1.248$, Spearman $\rho = 0.176$
(vs.\ trivial baselines 1.066 / 0.977); only 3/9 folds beat the
global-mean baseline; prior affinity $\rho \approx 0.93$ does not reproduce.
Model~D LOLO CSI Spearman is $\rho = -0.60$ ($n{=}5$, $p{=}0.28$);
prior CSI $\rho \approx 0.95$ does not reproduce.
Pooled Vinardo CSI after corrected PGG docks: PGG\,1.21, ellagic\,1.05
(neither CSI\,$< 1$).
PGG IG recomputed on 67 heavy atoms ($\bar{I} \approx 0.033$).
See \texttt{docs/REMEDIATION\_STATUS.md} in the code repository.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Model~D} trains on a smaller subset (5,267 samples) due
    to the MMP-1 data requirement. Post--B6 Model~D LOLO CSI Spearman
    is $\rho = -0.60$ ($n{=}5$, $p{=}0.28$); the previously reported
    $\rho = 0.95$ does \emph{not} reproduce and is withdrawn
    (Model~A affinity LOLO $\rho \approx 0.18$).
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  \item Compact phenols retain higher mean per-atom importance than PGG
    after B6 correction (PGG $N{=}67$, catalogue C$_{41}$H$_{32}$O$_{26}$).
  \item Prior SI rows quoting PGG $N{=}81$ referred to the wrong
    tri-$O$-galloyl fragment and are withdrawn.
