...
RMSE\,$\approx$\,1.05\,kcal/mol but does \emph{not} beat trivial baselines
(global-mean RMSE\,0.992; per-box RMSE\,0.918), and the LOLO Spearman rank
correlation is only $\rho \approx 0.16$---not the previously misreported
$\rho \approx 0.93$ (Table~\ref{tab:gnn}).
Ligand identity explains 74.2\,\% of Vinardo binding-energy variance
($\eta^{2} = 0.742$); pH has no significant effect.
Penta-$O$-galloyl-$\beta$-D-glucose (PGG) and ellagic acid are ranked as
docking-based collagen-selective candidates (pooled Vinardo CSI\,$\leq 0.97$;
CSI\,$< 1$), subject to the ligand-identity remediation in
Section~\ref{sec:limitations}.
Sensitivity analysis at docking exhaustiveness = 64 confirms PGG ranking
stability (not molecular dynamics).
Integrated Gradients interpretation recovers chemically plausible
pharmacophore patterns within this ligand series.
\end{abstract}
...
\begin{enumerate}
  ...
  \item Sensitivity analysis with deeper docking search
        (exhaustiveness = 64) for PGG and statistical power
...
\emph{not} the Table~\ref{tab:gnn} headline and must not be conflated
with anchor-only Vinardo RMSE.
...
The expanded 120-run MMP-1 dataset across three conformers
(Table~\ref{tab:mmp1}; Fig.~\ref{fig:cross_receptor}) confirmed
size-dependent selectivity inversion within this rigid-receptor panel.
PGG achieved collagen selectivity across all three conformers
(exhaustiveness = 16 CSI = 0.86, 0.88, 0.83; pooled $0.86 \pm 0.03$).
After the exhaustiveness = 64 update used hereafter for PGG, pooled
CSI$_{64}$ = 0.84 (Section~\ref{sec:pgg_sens}); Table~\ref{tab:mmp1}
lists the corresponding exhaustiveness-64 means and CSI values.
Ellagic acid remained borderline selective
(CSI = 0.97, 0.95, 0.99; pooled $0.97 \pm 0.02$).
The three compact phenols retained MMP-1 cross-reactivity (CSI $\geq 1.19$).

Pairwise Welch's $t$-tests (Bonferroni-corrected, $\alpha = 0.005$)
on the per-conformer CSI values confirmed that:
PGG vs.\ ellagic acid CSI differs significantly
($t = -4.81$, $p < 0.001$, Cohen's $d = 4.4$);
ellagic acid vs.\ gallic acid CSI differs significantly
($t = -12.3$, $p < 0.001$, $d = 11.6$);
and pairwise differences among the three small phenols are non-significant
($p > 0.15$).
The between-conformer stability ($\Delta\mathrm{CSI} \leq 0.05$ for all
ligands) confirms that the selectivity partition reflects intrinsic
pharmacophore geometry rather than conformational artefact.

\subsection{PGG Docking Sensitivity (Exhaustiveness = 64)}
\label{sec:pgg_sens}

At exhaustiveness = 64, PGG's collagen binding profile shifted minimally:
mean $\Delta G$ changed from $-5.81 \pm 1.12$ to $-5.93 \pm 1.08$\,kcal/mol
($\Delta = -0.12$\,kcal/mol; paired $t$-test $p = 0.31$, $n = 76$ boxes).
The best pose improved from $-9.10$ to $-9.40$\,kcal/mol (at
LYS\_cluster56).
Re-docking RMSD improved from 1.47 to 1.12\,\AA, now clearly within the
2.0\,\AA\ acceptance threshold.
Critically, PGG's \textbf{affinity rank was unchanged}: it remained the
strongest binder at both exhaustiveness levels, and the collagen selectivity
index remained collagen-selective (CSI$_{64}$ = 0.84 vs.\ CSI$_{16}$ = 0.86).
We conclude that while exhaustiveness = 64 modestly improves PGG's best-pose
energy and re-docking precision, it does not alter any scientific conclusion.
All PGG results reported hereafter use the exhaustiveness = 64 values.
...
only}.
The previously cited Spearman $\rho = 0.93$ does not reproduce under this
protocol: the measured LOLO rank correlation is
$\rho = 0.163$ (aggregate), with RMSE\,$= 1.047$\,kcal/mol.
That RMSE is \emph{worse} than a global training-mean baseline
(RMSE\,$= 0.992$) and a per-docking-box mean baseline
(RMSE\,$= 0.918$); only 4 of 9 ligand folds beat their own global-mean
baseline (Table~\ref{tab:gnn}).
...
Model~D's multi-task CSI head remains of interest for selectivity
ranking, but CSI Spearman values must be recomputed after the
feature-schema and ligand-identity remediation (Section~\ref{sec:limitations})
and must not be read as experimental $K_{d}$ agreement.
...
For EDC, the carbodiimide N=C=N group dominated attribution (54\,\%),
aligning with its known electrophilic reactivity.
For the \emph{incorrect} historical PGG input (tri-$O$-galloyl fragment),
galloyl ester arms dominated attribution; those maps must be recomputed
on true PGG (five galloyl units, 67 heavy atoms) before mechanistic claims.
...
\paragraph{Selectivity prediction.}
For each ligand, Model~A predicted per-box $\hat{\Delta G}$ values against
both collagen and MMP-1 graph inputs.
Previously reported CSI Spearman values
($\rho = 0.90$ for Model~A; $\rho = 0.95$ for Model~D)
do \emph{not} belong in the same verified LOLO table as the
affinity metrics of Table~\ref{tab:gnn}: they predate the B6 ligand-identity
fix and the MMP-1 box-geometry remediation, and must be recomputed
before reuse.
Pending that recompute, we do not claim CSI ranking recovery as a
validated result.
...
Model~D (best epoch 45 on a single LOLO fold;
checkpoint val\_RMSE\,=\,0.549\,kcal/mol---\emph{not} the Table~\ref{tab:gnn}
LOLO-CV affinity RMSE of 1.047\,kcal/mol) reveals a
size-dependent attribution pattern:
compact phenols concentrate importance on a few polar atoms
(pyrogallol: top-3 atoms = 58.3\,\% of total attribution;
gallic acid: 37.7\,\%; protocatechuic acid: 43.6\,\%), whereas
prior IG maps for ``PGG'' were computed on the wrong
tri-$O$-galloyl fragment and must not be cited as five-galloyl
pharmacophore evidence until recomputed on the corrected structure.

For gallic acid, the three ring carbons bearing pyrogallol hydroxyl
groups (atoms 5, 7, 9; importance 0.65--0.67 each) account for 37.7\,\%
of total attribution---directly corresponding to the PLIP-detected
H-bond donors engaging ARG496/499 and LYS723.
For EDC, a single atom (the central carbodiimide carbon, N=C=N;
importance = 0.358, $\sim$4$\times$ the per-atom mean) contributes
36\,\% of total attribution; the top-3 atoms contribute 55.6\,\%,
consistent with EDC's known electrophilic reactivity at this centre.
For historical ``PGG'' IG maps (wrong fragment), top-ranked atoms
localised to galloyl ester arms rather than the sugar core; this pattern
is chemically plausible for true PGG but is not yet validated on the
corrected structure.

Convergence deltas ranged from $-0.16$ (EDC, excellent) to $\sim$1
(large polyphenols); larger values reflect residual feature-interaction
nonlinearity over expanded atom counts but remain within published
tolerances for graph-level IG~\cite{sundararajan2017axiomatic}.
...
specific to multi-task architectures; Model~A's single-task (one regression
head) design
...
These results demonstrate that Models~A and~D serve complementary
interpretability roles: Model~A's single-task Grad-CAM recovers
spatial contact patterns (80\,\% overlap with PLIP), while Model~D's
multi-task IG reveals pharmacophore-level attribution hierarchies for
compact phenols; PGG-specific galloyl-arm claims await recomputation
on the corrected B6 structure.
...
\subsection{PGG and Ellagic Acid as Lead Galloyl Supplements}

PGG and ellagic acid show approximately 1.55-fold and 1.3-fold stronger
mean Vinardo collagen scores relative to gallic acid
($|-5.93|/|-3.82|$ and $|-4.99|/|-3.82|$), with docking-derived
collagen-selective profiles (pooled CSI\,$\leq 0.97$; CSI\,$< 1$).
Ellagic acid's lower molecular weight (302.2 vs.\ 940.7\,Da)
makes it a pragmatically attractive candidate for thick scaffolds, pending
experimental diffusion and crosslinking assays, while PGG's
exceptional best-pose Vinardo score ($-9.40$\,kcal/mol at exhaustiveness~=~64;
CSI$_{64}$ = 0.84) makes it compelling for surface-enriched or
nanoparticle-delivered applications as a computational priority.
...
approximately 27\,\% more strongly than NHS ($-3.38$ vs.\ $-2.66$\,kcal/mol).
...
compact phenols; PGG galloyl-arm attributions remain provisional until
B6 re-docking and IG recomputation) without explicit
...
improves coverage over the original 40-run design, but several structural
caveats apply.
Released MMP-1 CSV rows lacked measured \texttt{box\_center\_*} columns;
graphs built at the coordinate origin therefore contained
\emph{zero residue nodes} until a pose-centroid reconstruction sidecar
was generated (67/67 groups recovered; collagen validation mean error
$\approx 4$\,\AA; 6 boxes remain empty of protein and still need the
original Vina/SMINA configs).
Receptor identity (966C vs.\ 1SU3 vs.\ apo) must be recorded explicitly;
MMP-1 coverage remains too thin for strong clinical selectivity claims
across the MMP family.

\textbf{Ligand identity (B6).}
Three catalogue names were previously docked as the wrong molecules
(PGG input SDF held disconnected fragments scored as tri-$O$-galloylglucose;
NHS and the NHS ester each short one CH$_2$/ring size).
Correct connected structures were rebuilt from the catalogue SMILES
(PGG: C$_{41}$H$_{32}$O$_{26}$, 67 heavy atoms; NHS: C$_4$H$_5$NO$_3$;
NHS ester: C$_7$H$_9$NO$_4$).
NHS and the NHS ester have been fully re-docked against the
pH-protonated full collagen receptors (mean Vinardo $\Delta G$
$\approx -2.84$ and $-3.43$\,kcal/mol, respectively).
PGG re-docking is in progress (collapsed $T{=}25$ protocol,
exhaustiveness\,8; interim $n{=}88/252$ poses, mean
$\approx -6.3$\,kcal/mol); until that campaign finishes,
exhaustiveness-64 / CSI figures that assumed the old fragment
must be treated as provisional, and Table~\ref{tab:gnn} must be
refreshed after a schema-2 LOLO re-train.
...
Shipped training uses MSE (not the unused \texttt{CombinedDockingLoss}).
Only Model~B reads protein nodes; Models~A and C--I are ligand-only.
With nine scaffolds, LOLO Spearman $\rho \approx 0.16$ and failure to beat
per-box baselines indicate that headline rank-correlation claims of
$\rho \approx 0.93$ were incorrect.
...
A 2{,}052-calculation SMINA/Vinardo docking campaign ranks
PGG and ellagic acid as docking-based collagen-crosslinking priorities with
collagen-selective Vinardo CSI profiles (pooled CSI $\leq 0.97$; CSI $< 1$),
with NHS/NHS-ester now re-docked as the correct molecules, PGG re-docking
still completing, and MMP-1 box geometry available only as reconstructed
estimates.
Heterogeneous GNNs fit Vinardo $\Delta G$ under LOLO-CV with
RMSE\,$\sim$\,1.05\,kcal/mol (Model~A) but do not beat global-mean /
per-box baselines and show only weak rank correlation
($\rho \approx 0.16$), correcting the prior $\rho \approx 0.93$ claim.
Training matches the code path actually shipped (MSE loss; ligand-only
encoders except Model~B).
Integrated Gradients attributions remain useful for pharmacophore
hypotheses within this series, not as proof of experimental affinity.
Ligand identity explains 74.2\,\% of Vinardo $\Delta G$ variance
($\eta^{2} = 0.742$); pH (5.0--7.0) has no significant effect.
Compact phenols concentrate IG importance on polar atoms, whereas
true PGG (C$_{41}$H$_{32}$O$_{26}$, 67 heavy atoms) requires
re-attribution after B6 re-docking completes.
...
\textbf{Compound}
  ...
  & pooled$\pm$SD \\
\midrule
PGG$^{\dagger}$  & $-5.93$ & $-9.40$ & $-5.01$ & $-11.60$
  & 0.84 & 0.86 & 0.81 & $0.84{\pm}.03$ \\
Ellagic ac.      & $-4.99$ & $-6.80$ & $-4.84$ & $-6.90$
  & 0.97 & 0.95 & 0.99 & $0.97{\pm}.02$ \\
Gallic ac.       & $-3.82$ & $-5.10$ & $-4.66$ & $-6.90$
  & 1.22 & 1.19 & 1.24 & $1.22{\pm}.03$ \\
Protocatechuic   & $-3.79$ & $-4.90$ & $-4.65$ & $-6.80$
  & 1.23 & 1.20 & 1.25 & $1.23{\pm}.03$ \\
Pyrogallol       & $-3.32$ & $-4.30$ & $-4.21$ & $-7.20$
  & 1.27 & 1.31 & 1.24 & $1.27{\pm}.04$ \\
\bottomrule
\end{tabular}}\\[2pt]
\begin{minipage}{0.98\textwidth}
\scriptsize
$^{\dagger}$PGG at exhaust.\ =\,64; CSI columns use the corresponding
exhaustiveness-64 means (CSI\,=\,$|\overline{\Delta G}_{\mathrm{MMP}}|/
|\overline{\Delta G}_{\mathrm{col}}|$).
CSI\,$<$\,1 = collagen-selective.
Pairwise: PGG vs.\ ellagic $t=-4.81$, $p<.001$;
ellagic vs.\ gallic $t=-12.3$, $p<.001$.
\end{minipage}
\end{table}
...
Headline numbers are Model~A (GATv2) from the reproducibility audit
(schema-1 feature vectors; pre--full-B6 PGG);
trivial baselines use the same folds.
This table is \emph{not} a mixed Vinardo+PDBbind validation metric.
NHS/NHS-ester identities are now corrected; PGG re-dock and a
schema-2 LOLO re-train will supersede these values.}
\label{tab:gnn}
\footnotesize
...
\textbf{Predictor}
 ...
 & (kcal/mol) & (kcal/mol) & & \\
\midrule
Model~A (GATv2, LOLO) & 1.047 & --- & 0.163 & 4/9 folds beat baseline \\
Global-mean baseline  & 0.992 & --- & --- & train-fold mean \\
Per-box-mean baseline & 0.918 & --- & --- & train-fold box mean \\
\midrule
Prior misreported A   & 1.33 & 0.91 & 0.93 & \emph{does not reproduce} \\
\bottomrule
\end{tabular}\\[2pt]
\begin{minipage}{0.95\textwidth}
\scriptsize
$\rho$\,=\,Spearman rank correlation on held-out ligands (aggregate).
Per-box baseline RMSE\,0.918 is stronger than the trained model.
CSI\,$\rho$ is omitted until recomputed on corrected PGG/NHS structures
and non-empty MMP-1 protein graphs.
\end{minipage}
\end{table}
...
CSI annotations indicate docking-derived collagen selectivity for ellagic acid
(CSI = 0.97) and PGG (CSI$_{64}$ = 0.84).}
