...
$K_{d}$).
Under leave-one-ligand-out cross-validation (LOLO-CV) after B6 ligand
re-docking (schema-2 features), Model~A reaches
RMSE\,$\approx$\,1.34\,kcal/mol but does \emph{not} beat trivial baselines
(global-mean RMSE\,1.066; per-box RMSE\,0.977), and the LOLO Spearman rank
correlation is only $\rho \approx 0.18$---not the previously misreported
...
Penta-$O$-galloyl-$\beta$-D-glucose (PGG) remains the strongest collagen
binder after ligand-identity remediation (mean Vinardo
$\Delta G \approx -6.34$\,kcal/mol), but pooled CSI\,$= 1.21$
(CSI\,$> 1$) indicates MMP-1 preference under this rigid-receptor panel;
prior CSI\,$< 1$ claims for PGG/ellagic do not hold on corrected poses
(Section~\ref{sec:mmp1}).
Model~D LOLO CSI Spearman is $\rho = -0.60$ ($n{=}5$, $p{=}0.28$), not
the previously claimed $0.95$.
Integrated Gradients on corrected PGG (67 heavy atoms) yields diffuse
per-atom importance consistent with size dilution of the galloyl signal.
\end{abstract}
...
After B6 re-docking of catalogue-correct PGG (and NHS/NHS-ester), the
expanded 120-run MMP-1 panel (Table~\ref{tab:mmp1};
Fig.~\ref{fig:cross_receptor}) \emph{no longer} supports collagen
selectivity for PGG or ellagic acid.
Pooled Vinardo CSI (T\,=\,25\,$^{\circ}$C collagen means vs.\ MMP-1) is
1.21 for PGG and 1.05 for ellagic acid (CSI\,$> 1$ = MMP-1 preference);
only the 966C ellagic conformer remains borderline collagen-selective
(CSI\,=\,0.97).
The three compact phenols remain MMP-1-preferring (CSI\,$\geq 1.20$).
Historical CSI\,$\approx 0.84$--$0.97$ values for PGG/ellagic were
computed on the wrong PGG fragment and must not be reused.
Between-conformer CSI spreads remain modest
($\Delta\mathrm{CSI} \lesssim 0.17$), but the partition no longer
separates a collagen-selective galloyl class from phenols.

\subsection{PGG Docking After Ligand-Identity Correction}
\label{sec:pgg_sens}

Catalogue-correct PGG (C$_{41}$H$_{32}$O$_{26}$, 67 heavy atoms) was
re-docked on full pH-protonated collagen receptors (collapsed $T{=}25$
protocol, exhaustiveness\,8) and against reconstructed MMP-1 boxes.
Mean collagen Vinardo $\Delta G \approx -6.34$\,kcal/mol ($n{=}684$;
T\,=\,25 subset mean identical within rounding), best pose
$\approx -9.20$\,kcal/mol; MMP-1 mean $\approx -7.68$\,kcal/mol
(pooled CSI\,=\,1.21).
PGG remains the strongest collagen binder in the nine-ligand panel, but
is \emph{not} collagen-selective under the CSI\,$< 1$ criterion.
Prior exhaustiveness-64 numbers (mean $\approx -5.93$, CSI$_{64}\approx 0.84$)
refer to the incorrect tri-$O$-galloyl fragment and are retained only as
historical context.
...
protocol: after corrected NHS/NHS-ester/PGG docks and schema-2 features,
the measured LOLO rank correlation is
$\rho = 0.176$ (aggregate), with RMSE\,$= 1.338$\,kcal/mol
(MAE\,$= 1.248$).
...
$\rho = -0.60$ ($n{=}5$, $p{=}0.28$) after B6 remediation---the prior
$\rho = 0.95$ claim does not reproduce and must not be read as
...
On catalogue-correct PGG (67 heavy atoms), mean per-atom IG importance
is low ($\bar{I} \approx 0.033$) with total molecular attribution
still large ($67 \times 0.033 \approx 2.2$), consistent with
size-diluted galloyl contributions rather than a single hot atom.
...
do \emph{not} reproduce after B6 ligand-identity and MMP-1 box remediation
and are withdrawn.

\paragraph{Model~D multi-task interpretability.}
Integrated Gradients (IG; 50 interpolation steps, zero-feature baseline)
were recomputed on Model~D's collagen $\Delta G$ head for all nine ligands
using the PGG held-out LOLO checkpoint (best epoch\,10;
val\_RMSE\,=\,0.403\,kcal/mol---\emph{not} the Table~\ref{tab:gnn}
Model~A LOLO affinity RMSE).
Compact phenols retain higher mean per-atom importance than PGG
(gallic $\bar{I} = 0.177$; protocatechuic $0.196$; pyrogallol $0.225$;
PGG $0.033$ on 67 atoms).
Prior IG tables quoting PGG $N{=}81$ referred to the wrong fragment and
are superseded (Supporting Information Appendix~D).
...
\subsection{PGG as Affinity Lead; Selectivity Caveat}

After B6 correction, PGG remains the strongest collagen binder
...
However, pooled CSI\,$> 1$ for PGG (1.21) and ellagic acid (1.05)
means neither is collagen-selective under this rigid MMP-1 panel;
they remain computational affinity priorities, not CSI\,$< 1$ leads.

\subsection{Reaction Intermediate Binding}

The EDC-$O$-acylisourea intermediate binds approximately 17\,\% more strongly
than parent EDC ($-3.64$ vs.\ $-3.11$\,kcal/mol), and the corrected NHS-ester
approximately 21\,\% more strongly than corrected NHS ($-3.43$ vs.\ $-2.84$\,kcal/mol).
...
NHS ester: C$_7$H$_9$NO$_4$) and re-docked against the pH-protonated
full collagen receptors (PGG also vs.\ MMP-1 with reconstructed boxes;
collapsed $T{=}25$ protocol at exhaustiveness\,8 for PGG).
Mean Vinardo $\Delta G$ after correction:
NHS $\approx -2.84$, NHS ester $\approx -3.43$,
PGG $\approx -6.34$\,kcal/mol ($n{=}684$ collagen rows).
Prior exhaustiveness-64 CSI figures that assumed the old PGG fragment are
withdrawn.
Post--B6 schema-2 LOLO (Table~\ref{tab:gnn}) and Model~D CSI/IG
recomputes supersede those claims: Vinardo CSI\,$> 1$ for corrected PGG,
and CSI Spearman $\rho = -0.60$ ($n{=}5$).
...
With nine scaffolds, LOLO affinity Spearman $\rho \approx 0.18$, CSI
Spearman $\rho = -0.60$, and failure to beat per-box baselines indicate
that headline claims of affinity $\rho \approx 0.93$ and CSI $\rho \approx 0.95$
were incorrect.
...
A 2{,}052-calculation SMINA/Vinardo docking campaign, after correcting
NHS/NHS-ester/PGG identities, ranks PGG as the strongest collagen binder
(mean $\Delta G \approx -6.34$\,kcal/mol) but \emph{not} as
collagen-selective under CSI\,$< 1$ (pooled CSI\,=\,1.21); ellagic acid
is similarly non-selective (CSI\,=\,1.05).
MMP-1 box geometry remains reconstructed.
Heterogeneous GNNs fit Vinardo $\Delta G$ under LOLO-CV with
RMSE\,$\sim$\,1.34\,kcal/mol (Model~A) but do not beat global-mean /
per-box baselines and show only weak affinity rank correlation
($\rho \approx 0.18$), correcting the prior $\rho \approx 0.93$ claim.
Model~D CSI Spearman is $\rho = -0.60$ ($n{=}5$), correcting the prior
$\rho \approx 0.95$ claim.
Training matches the code path actually shipped (MSE loss; ligand-only
encoders except Model~B).
Integrated Gradients on corrected PGG (67 heavy atoms) shows
size-diluted per-atom importance ($\bar{I} \approx 0.033$).
...
\begin{minipage}{0.98\textwidth}
\scriptsize
$^{\dagger}$Catalogue-correct PGG after B6 re-dock (exhaust.\ =\,8,
collapsed $T{=}25$); CSI\,=\,$|\overline{\Delta G}_{\mathrm{MMP}}|/
...
(0.97); prior PGG CSI\,$\approx 0.84$ values are withdrawn.
\end{minipage}
\end{table}
...
Headline numbers are Model~A (GATv2) after B6 NHS/NHS-ester/PGG re-dock
and schema-2 features (CPU, seed~0, 50 epochs);
...
Prior misreported A   & 1.33 & 0.91 & 0.93 & affinity $\rho$; \emph{does not reproduce} \\
Prior misreported D   & --- & --- & 0.95 & CSI $\rho$; \emph{does not reproduce} \\
\bottomrule
\end{tabular}\\[2pt]
...
CSI annotations use post--B6 pooled values (ellagic CSI\,=\,1.05;
