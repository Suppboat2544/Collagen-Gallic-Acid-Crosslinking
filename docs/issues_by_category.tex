...
\date{2026-08-17 (register); remediation status updated 2026-09-09}

\begin{document}
\sloppy
\maketitle

\noindent\textbf{Remediation note (2026-09-09).}
Blocking code defects (P1.1--P1.4, \ldots) remain \SFIXED{}.
Manuscript metrics and ligand-identity items below marked
\SFIXED{} in this update: \textbf{P1.5}, \textbf{P1.6}, \textbf{P4.1},
\textbf{P4.2}, \textbf{P4.4}, \textbf{P4.5}, \textbf{P4.7}, \textbf{B4.1}
(reconstructed boxes; 6 still empty), \textbf{B6.1--B6.3}.
Post--B6 schema-2 LOLO (Model~A): RMSE\,$= 1.338$, MAE\,$= 1.248$,
$\rho = 0.176$ vs.\ baselines 1.066 / 0.977.
Model~D CSI Spearman $\rho = -0.60$ ($n{=}5$); prior $0.95$ withdrawn.
Vinardo CSI for corrected PGG\,$= 1.21$ (not $< 1$).
See \texttt{docs/REMEDIATION\_STATUS.md}.
...
\vspace{4pt}\noindent
\textbf{Headline (audit, pre--full-B6):} under leave-one-ligand-out
cross-validation on the then-released data, Model~A reached Spearman
$\rho = 0.163$ against a reported $0.93$, with RMSE $1.047$ against a
global-mean baseline of $0.992$ and a per-docking-box baseline of $0.918$.
\textbf{Headline (post--B6 schema-2, 2026-09):} Model~A RMSE\,$= 1.338$,
MAE\,$= 1.248$, $\rho = 0.176$ vs.\ baselines 1.066 / 0.977;
Model~D CSI $\rho = -0.60$ ($n{=}5$). Prior $0.93$ / $0.95$ claims
remain withdrawn.
...
P1.5 & \SMAJOR{} \SFIXED{} Published metrics mix two target quantities &
  Historical mixing of PDBbind into validation is documented.
  Manuscript Table~IV is now restricted to \emph{anchor Vinardo-only}
  LOLO; PDBbind-mixed numbers are excluded from the verified table. \\
P1.6 & \SMAJOR{} \SFIXED{} Documented loss never executed &
  Training still uses \code{nn.MSELoss()} only; \code{CombinedDockingLoss}
  remains dead code. The manuscript now states MSE (not smooth-L1 /
  CombinedDockingLoss), matching the executed path. \\
...
P1.8 & \SMAJOR{} \SFIXED{} Shipped cache is stale &
...
P2.5 & \SMINOR{} \SFIXED{} Hardware claim &
...
\midrule
\endhead
P4.1 & \SBLOCK{} \SFIXED{} Rank correlation does not reproduce &
  Manuscript now reports measured LOLO $\rho$ (post--B6 schema-2:
  Model~A $\rho = 0.176$) and withdraws the prior $0.93$ claim.
  Audit-era value on pre--full-B6 data was $\rho = 0.163$. \\
P4.2 & \SBLOCK{} \SFIXED{} No baselines reported &
  Baselines are published beside Model~A (post--B6: global-mean RMSE
  $1.066$, per-box $0.977$; Model~A $1.338$, only 3/9 folds beat
  global-mean). \\
...
P4.4 & \SMAJOR{} \SFIXED{} Eight of nine models never read the protein &
  Code unchanged (only Option~B reads protein nodes). Manuscript now
  states this explicitly. \\
P4.5 & \SMAJOR{} \SFIXED{} Models F and G have no real geometry &
  Code unchanged (pseudo-geometry). Manuscript now states that F/G
  equivariant biases are only partially exercised. Underlying defect:
  EGNN coordinates are \code{Linear(35$\to$3)}; DimeNet angles from
  \code{cosine\_similarity} of edge features. Neither
...
P4.7 & \SMAJOR{} \SFIXED{} Model D checkpoint misread &
...
\midrule
\endhead
B4.1 & \SBLOCK{} \SFIXED{} Every MMP-1 record has an empty receptor &
...
B4.2 & \SBLOCK{} \SFIXED{} Root cause: missing box geometry &
...
\midrule
\endhead
B6.1 & \SBLOCK{} \SFIXED{} Pentagalloylglucose &
...
\noindent\textbf{Status 2026-09:} catalogue structures rebuilt and re-docked on full collagen receptors (PGG also vs.\ MMP-1). Mean collagen $\Delta G$: NHS $\approx -2.84$, NHS ester $\approx -3.43$, PGG $\approx -6.34$\,kcal/mol. Pooled Vinardo CSI for PGG $= 1.21$ (not collagen-selective). Provenance check: 9/9 ligands match docked structures.\par\bigskip
...
\begin{enumerate}\itemsep2pt
  \item \textbf{P4.1, P4.2, P1.5, P1.6, P4.4, P4.5, B6.1--B6.3} ---
        \emph{done} (2026-09): manuscript Table~IV / CSI / IG updated;
        NHS/NHS-ester/PGG re-docked; schema-2 LOLO + CSI Spearman + PGG IG
        recomputed. See \texttt{docs/REMEDIATION\_STATUS.md}.
  \item \textbf{B3.1, B2.1} --- \emph{done}: pH table and galloyl SMARTS
        corrected (feature schema~2).
  \item \textbf{B4.2 (residual)} --- optional: obtain original MMP-1 Vina
        configs for the 6 boxes that remain empty after pose-centroid
        reconstruction (67/67 groups recovered; ${\sim}4$\,\AA{} collagen
        validation error).
  \item \textbf{P4.5 (code)} --- optional: wire real ETKDG coordinates into
        Models F/G (manuscript caveat already present).
  \item \textbf{Figures} --- refresh any PDF figures that still annotate
        pre-B6 CSI values.
\end{enumerate}
